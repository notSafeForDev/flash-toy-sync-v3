...
class utils.MathUtil {
	...
	public static function wrap(_value : Number, _min : Number, _max : Number) : Number {
		...
		if (_min > _max) {
			var returnValue; trace("Error: " + "Unable to calculate wrapped value, min must be greater or equal to max"); return returnValue;
		}
	...
	public static function getPercentage(_value : Number, _from : Number, _to: Number) : Number {
		...
		if (_from == _to) {
			var returnValue; trace("Error: " + "Unable to calcuate percentage, both from and to values are the same"); return returnValue;
		}
