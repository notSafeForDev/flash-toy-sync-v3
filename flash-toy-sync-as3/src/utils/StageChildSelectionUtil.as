package utils {
	...
	public class StageChildSelectionUtil {
		...
		public static function getClickableChildAtCursor(_topParent : DisplayObjectContainer) : DisplayObject {
			var child : DisplayObject = ScenesState.selectedChild.value || _topParent;
			var children : Array = DisplayObjectUtil.getChildren(child);
			...
			while (isDone == false) {
				...
				for (var i : Number = children.length - 1; i >= 0; i--) {
					...
					var childBounds : Rectangle = DisplayObjectUtil.getBounds(children[i], DisplayObjectUtil.getParent(_topParent));
				...
				if (closestChild != null) {
					...
					bounds = DisplayObjectUtil.getBounds(child, DisplayObjectUtil.getParent(_topParent));
				} else {
					isDone = true;
				}
