package core {
	...
	public class CustomEvent {
		 ...
		 */
		public function listen(_scope : * , _handler : Function, ...rest) : Object {
			var handler : Function = _handler;
			
			if (rest.length > 0) {
				handler = function(...rest2) : void {
					_handler.apply(_scope, rest.concat(rest2));
				}
			}
		 ...
		 * @param	rest 	Any values to send to the callbacks
		 */
		public function emit(...rest) : void {
			for (var i : Number = 0; i < listeners.length; i++) {
				var listenerFunction : Function = this.listeners[i].handler;
				listenerFunction.apply(listeners[i].scope, rest);
